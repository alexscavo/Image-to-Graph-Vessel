...
class SetCriterion(nn.Module):
    ...
    def __init__(self, config, matcher, net, num_edge_samples=80,
                edge_sampling_mode=EDGE_SAMPLING_MODE.UP,
                domain_class_weight=None,
                ema_relation_embed=None):
        ...
        if config.DATA.MIXED:
            ...
            print('-'*50)
            print('USING ADVERSARIAL NETWORK') if config.TRAIN.GRAPH_ADVERSARIAL else print('NO ADVERSARIAL NETWORK')
        else:
    ...
    def select_hard_negative_edges(
        ...
        # --- EMA scoring ---
        logits = self.ema_relation_embed.ema(relation_feature)  # logits for each negative edge in cand_size
    ...
    def loss_edges(self, h, target_nodes, target_edges, indices):
        ...
        for batch_id, (pos_edge, n) in enumerate(zip(new_target_edges, target_nodes)):
            ...
            try: 
                ...
                print('-'*50)
                print(e)
                print('full_adj shape:', full_adj.shape)
                print('pos_edge shape:', pos_edge.shape)
                print('-'*50)
