...
def test(args):
    ...
    viz = DebugVisualizer(out_dir=out_dir, prob=config.display_prob)  # prob=1.0 since we handle prob here
    viz.start_epoch()
    current_fold = 0
    ...
    with torch.no_grad():
        ...
        for i, batchdata in enumerate(tqdm(test_loader)):
            ...
            viz.maybe_save(
                images_bchw=images,              # (B,C,H,W), on CUDA; the class detaches/CPU's it
                gt_nodes_list=nodes,             # list of length B of torch tensors
                gt_edges_list=edges,             # list of length B of torch tensors
                pred_nodes_list=pred_nodes,      # list of length B (torch or np is fine)
                pred_edges_list=pred_edges,      # list of length B
