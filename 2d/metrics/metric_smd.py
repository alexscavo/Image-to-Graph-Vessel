...
class MeanSMD(Metric):
    ...
    def update(self, output) -> None:
        ...
        gt_nodes, gt_edges, pred_nodes, pred_edges = output
        
        return self.metric_fn(gt_nodes, gt_edges, pred_nodes, pred_edges)
...
class StreetMoverDistance(ABC):
    ...
    def _compute_list(self, node_list, edge_list, pred_node_list, pred_edge_list):
        ...
        for nodes, edges, pred_nodes, pred_edges in zip(node_list, edge_list, pred_node_list, pred_edge_list):
            ...
            if nodes.shape[0]>1 and pred_nodes.shape[0]>1 and pred_edges.size != 0:
                ...
                # DEBUG
                # debug = compute_meanSMD(
                    # A.T, nodes, pred_A.T, pred_nodes, self.sinkhorn_distance, n_points=100
                # )
                
                # print('~'*50)
                # print('smd metric on prediction-prediction')
                # print(debug)
                # print('~'*50)
    ...
    #     ):
    #         # ensure tensors (handles numpy arrays transparently)
    #         nodes = torch.as_tensor(nodes)
    #         pred_nodes = torch.as_tensor(pred_nodes)

    #         edges_t = torch.as_tensor(edges, dtype=torch.long, device=nodes.device)
    #         pred_edges_t = torch.as_tensor(pred_edges, dtype=torch.long, device=pred_nodes.device)

    #         # ---- sort GT edges: each edge -> (min_idx, max_idx) ----
    #         if edges_t.numel() > 0:
    #             edges_sorted, _ = torch.sort(edges_t, dim=1)  # (E, 2)
    #         else:
    #             edges_sorted = edges_t

    #         A = torch.zeros((nodes.shape[0], nodes.shape[0]), device=nodes.device)
    #         if edges_sorted.numel() > 0:
    #             A[edges_sorted[:, 0], edges_sorted[:, 1]] = 1.0

    #         # ---- sort Pred edges ----
    #         if pred_edges_t.numel() > 0:
    #             pred_edges_sorted, _ = torch.sort(pred_edges_t, dim=1)
    #         else:
    #             pred_edges_sorted = pred_edges_t

    #         pred_A = torch.zeros((pred_nodes.shape[0], pred_nodes.shape[0]), device=pred_nodes.device)
    #         if pred_edges_sorted.numel() > 0:
    #             pred_A[pred_edges_sorted[:, 0], pred_edges_sorted[:, 1]] = 1.0

    #         # same condition as before, but on the tensor version
    #         if nodes.shape[0] > 1 and pred_nodes.shape[0] > 1 and pred_edges_sorted.numel() > 0:
    #             ret.append(
    #                 compute_meanSMD(
    #                     A.T, nodes, pred_A.T, pred_nodes, self.sinkhorn_distance, n_points=100
    #                 )
    #             )
    #         else:
    #             ret.append(torch.tensor([1.0], dtype=torch.float, device=nodes.device))
...
def compute_meanSMD(y_A, y_nodes, output_A, output_nodes, sinkhorn_distance, n_points=100):
    """
    y_A:         [Ny, Ny] adjacency (GT)
    y_nodes:     [Ny, D]  node coordinates (GT)
    output_A:    [No, No] adjacency (prediction)
    output_nodes:[No, D]  node coordinates (prediction)
    sinkhorn_distance:  callable that takes (P, Q) -> (dist, P_mat, C_mat)
    n_points:    approximate total #points in each point cloud
    """
