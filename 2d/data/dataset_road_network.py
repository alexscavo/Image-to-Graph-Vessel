...
class Sat2GraphDataLoader(Dataset):
    ...
    """
    def _get_edge_severity(self, idx):
        if idx in self._edge_sev_cache:
            return self._edge_sev_cache[idx]

        data = self.data[idx]
        # load nodes + lines ONLY (lightweight compared to full image)
        if data['vtp'].endswith('.vtp'):
            ...
            raw_lines = torch.tensor(np.asarray(vtk_data.lines), dtype=torch.int64)
            edges_e2 = _to_edge_index(raw_lines)
        else:
            nodes_xy, edges_ix, _ = load_graph_pickle_generic(data["vtp"])
            nodes = torch.from_numpy(nodes_xy.astype(np.float32))
            edges_e2 = _to_edge_index(torch.from_numpy(edges_ix.astype(np.int64)))

        sev = _bridge_severity_for_edges(int(nodes.shape[0]), edges_e2)
    ...
    def precompute_edge_severity(self, verbose=False):
        if verbose:
            print(f"[Sat2GraphDataLoader] Precomputing edge severity for {len(self)} samples...")
        for idx in range(len(self)):
            _ = self._get_edge_severity(idx)
        if verbose:
            print("[Sat2GraphDataLoader] Done.")
    ...
    def __init__(self, data, augment, use_grayscale=False, domain_classification=-1):
        ...
        self._edge_sev_cache = {}  #cache for severity infos: idx -> tensor
    ...
    def __getitem__(self, idx):
        ...
        # convert ONLY for severity computation
        edges_e2 = _to_edge_index(lines)
        edge_sev = self._get_edge_severity(idx)  # aligns with edges_e2 order for this sample
        
        print('-'*50)
        print(edge_sev)
        sys.exit()

        return image_data, seg_data - 0.5, nodes_norm, edges_e2, edge_sev, self.domain_classification    
