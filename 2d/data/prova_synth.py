...
class Vessel2GraphDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print('-'*50)
        print('nodes before')
        print(torch.tensor(nodes.to_numpy()[:, :2])[:15])
        
        nodes = torch.tensor(nodes.to_numpy()[:, :2].astype(np.float32))
...
def build_synthetic_vessel_network_data(config, mode='train', split=0.8, max_samples=0, use_grayscale=False, domain_classification=-1, mixed=False, has_val=False):
    ...
    if mode == 'train':
        ...
        img_folder = os.path.join(config.DATA.TARGET_DATA_PATH, 'train/raw')
        lbl_folder = os.path.join(config.DATA.TARGET_DATA_PATH, 'train/labels')
        graphs_folder = os.path.join(config.DATA.TARGET_DATA_PATH, 'train/graphs')
        ...
        img_folder = os.path.join(config.DATA.TEST_DATA_PATH, 'test/raw')
        graphs_folder = os.path.join(config.DATA.TEST_DATA_PATH, 'test/graphs')
        lbl_folder = os.path.join(config.DATA.TEST_DATA_PATH, 'test/labels')
