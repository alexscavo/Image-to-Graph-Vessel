...
def _normalize_nodes_xy(nodes: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    xy = nodes[:, :2]
    lo = xy.min(0).values
    hi = xy.max(0).values
...
def test(args):
    ...
    # --- Keep only the first sample (the same patch used in training) ---
    test_ds = Subset(test_ds, [0])

    # --- Optional: print which patch is used for debugging ---
    sample = test_ds.dataset.data[0] if hasattr(test_ds, "dataset") else test_ds.data[0]
    print("Testing on single patch:")
    ...
    fold_size = max(1, int(len(test_ds) / 5))
    ...
    viz = DebugVisualizer(out_dir=out_dir, prob=0.001)  # prob=1.0 since we handle prob here
    viz.start_epoch()
    current_fold = 0
    ...
    with torch.no_grad():
        ...
        for i, batchdata in enumerate(tqdm(test_loader)):
            ...
            viz.maybe_save(
                images_bchw=images,     # (B,C,H,W), CUDA is fine; class will detach/cpu
                gt_nodes_list=nodes,     # list len B (torch tensors)
                gt_edges_list=edges,     # list len B (torch tensors)
                pred_nodes_list=pred_nodes,
                pred_edges_list=pred_edges,
                epoch=current_fold,      # use fold as "epoch" during testing
                step=i,                  # test iteration
                batch_index=0,
                tag="test"
            )
            ...
            #     G1.dges_from([tuple(e) for e in edges_sample.cpu().tolist()])
