...
def main(args):
    ...
    try:
        ...
        print('evaluator eseguito con val_loader terminato')
    except TypeError:
        # Fallback: some helpers encapsulate the loader inside the evaluator
        evaluator.run()
        print('evaluator eseguito senza niente')

    m = getattr(evaluator.state, "metrics", {}) or {}
    ...
    # primary: negative total loss
    primary = as_float(m.get("val_total_loss"))
    if primary is not None:
        primary = -primary
    else:
        # fallback: negative average of edge/node losses
        ve = as_float(m.get("val_edge_loss"))
        vn = as_float(m.get("val_node_loss"))
        primary = -( (ve + vn) / 2.0 ) if (ve is not None and vn is not None) else 0.0

    print(f"VAL_METRIC={primary:.6f}", flush=True)
...
if __name__ == '__main__':
    ...
    # --- PRE-TRAINING --- 
    # args = parser.parse_args(['--exp_name', 'pretraining_mixed_synth2',
    #                           '--config', '/home/scavone/cross-dim_i2g/configs/pretrained_config_2d_synth.yaml'
    #                          ])
    
    
    # --- FINE TUNING ---
    # args = parser.parse_args(['--exp_name', 'finetune_mixed_synth2',
    #                         '--config', '/home/scavone/cross-dim_i2g/configs/config_2d_synth.yaml',
    #                         '--resume', '/data/scavone/cross-dim_i2g/trained_weights/runs/pretraining_mixed_synth2_10/models/checkpoint_epoch=50.pt',
    #                         '--no_strict_loading'
    #                         ])
    ...
    # --- PRE-TRAINING --- 
    # args = parser.parse_args(['--exp_name', 'pretraining_mixed_real1',
    #                           '--config', '/home/scavone/cross-dim_i2g/configs/pretrained_config_2d_real.yaml'
    #                          ])
    
    
    # --- FINE TUNING ---
    # args = parser.parse_args(['--exp_name', 'PROVA',
    #                         '--config', '/home/scavone/cross-dim_i2g/configs/config_2d_real.yaml',
