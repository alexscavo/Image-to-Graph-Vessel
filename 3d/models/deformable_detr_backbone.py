...
from .models2D.position_encoding_2D import build_position_encoding
...
class Joiner(nn.Sequential):
    ...
    def __init__(self, backbone, position_embedding):
        ...
        # Just replicating with a new name to make changes in original code as minimal as possible
        self.num_features = self.num_channels[0]
...
def build_backbone(config):
    ...
    return_interm_layers = config.MODEL.PRE2D_BACKBONE.MASKS or (
        config.MODEL.PRE2D_BACKBONE.NUM_FEATURE_LEVELS > 1)
    backbone = Backbone(
        config.MODEL.PRE2D_BACKBONE.BACKBONE,
        config.MODEL.PRE2D_BACKBONE.BACKBONE_INIT_WEIGHTS,
        True,
        return_interm_layers,
        config.MODEL.PRE2D_BACKBONE.DILATION
    )
