...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, config, dims=3):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        out_nodes = outputs['pred_nodes'][...,:self.dims].flatten(0, 1)  # [batch_size * num_queries, 3]
...
def build_matcher(config, dims):
    if config.MODEL.MATCHER.NAME=='Hungarian':
        return HungarianMatcher(config, dims=dims)
