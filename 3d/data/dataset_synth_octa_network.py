...
class OCTALoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_data = torch.tensor(
            np.array(img_data), dtype=torch.float).unsqueeze(0)
        ...
        # The coordinates in this dataset have a different anchor than the other datasets, se we need to adjust for that by switching the x and y coordinates
...
def build_octa_network_data(config, mode='train', split=0.95, debug=False, gaussian_augment=False, rotate=False, continuous=False, max_samples=0, domain_classification=-1):
    """[summary]
