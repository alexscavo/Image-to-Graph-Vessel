...
class SimilarityMetricTSNE(Metric):
  ...
  def update(self, output):
    ...
    if self.base_metric is None:
      ...
      X = features[-1][domains == 0].clone().permute(0,2,3,4,1).flatten(start_dim=0, end_dim=3).detach().cpu().numpy()
      Y = features[-1][domains == 1].clone().permute(0,2,3,4,1).flatten(start_dim=0, end_dim=3).detach().cpu().numpy()
...
class SimilarityMetricPCA(Metric):
  ...
  def __init__(self, output_transform=lambda x:x, device="cpu", similarity_function=batch_cka, base_metric=None, max_datapoints=2*10000):
  ...
  def update(self, output):
    ...
    if self.base_metric is None:
      if self.num_source_datapoints < self.max_datapoints:
        X = features[domains == 0].clone().permute(0,2,3,4,1).flatten(start_dim=0, end_dim=3).detach().cpu()
        self._source_batches.append(X)
        self.num_source_datapoints += X.shape[0]
      if self.num_target_datapoints < self.max_datapoints:
        Y = features[domains == 1].clone().permute(0,2,3,4,1).flatten(start_dim=0, end_dim=3).detach().cpu()
