...
def relation_infer(h, out, model, obj_token, rln_token, apply_nms=True, thresh=0.5):
    ...
    for batch_id in range(h.shape[0]):
        ...
        if node_id.dim() !=0 and node_id.nelement() != 0 and node_id.shape[0]>1:
            ...
            if rln_token>0:
                relation_feature1 = torch.cat((object_token[batch_id,node_pairs[0],:], object_token[batch_id,node_pairs[1],:], relation_token[batch_id,...].flatten().unsqueeze(0).repeat(len(node_pairs_valid),1)), 1)
                relation_feature2 = torch.cat((object_token[batch_id,node_pairs[1],:], object_token[batch_id,node_pairs[0],:], relation_token[batch_id,...].flatten().unsqueeze(0).repeat(len(node_pairs_valid),1)), 1)
            else:
            ...
            try:
                relation_pred1 = model.module.relation_embed(relation_feature1).detach()
                relation_pred2 = model.module.relation_embed(relation_feature2).detach()
