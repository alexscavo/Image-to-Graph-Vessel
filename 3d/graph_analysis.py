...
def _bridge_metrics(n_nodes: int, undirected_edges: np.ndarray):
    ...
    for p, ch in bridges_child:
        ...
        # Removing bridge (p-ch) splits component into:
        # - DFS subtree rooted at ch: sub_size[ch]
        # - the rest: comp_size - sub_size[ch]
        a = sub_size[ch]
        b = comp_size - a
        # normalize to [0, 0.5]
...
def compute_graph_stats(
    ...
    n_bridges, bridge_inc_nodes, sev_mean, sev_max = _bridge_metrics(n_nodes, undirected_edges)  # from your new helper(s)
...
def analyze_split(
    ...
    for b_idx, batch in tqdm.tqdm(enumerate(loader), total=len(loader)):
        ...
        # Expected batch format (see trainer_with_ema.py _iteration):
        # images, segs, nodes, edges, z_pos, domains = batch
        # Some datasets might not have segs; still, nodes/edges/domains should be present.
        if not isinstance(batch, (list, tuple)) or len(batch) < 6:
            raise ValueError(
                f"Unexpected batch structure for split={split_name}. "
                f"Expected at least 6 elements (images,segs,nodes,edges,z_pos,domains). Got: {type(batch)} len={len(batch) if isinstance(batch, (list, tuple)) else 'n/a'}"
            )
...
def aggregate_stats(stats: List[GraphStats]) -> Dict[str, Any]:
    ...
    # per-domain aggregates (if domains look valid)
...
def main(args):
    ...
    # Basic validation similar to train script
    if not getattr(config.DATA, "SOURCE_DATA_PATH", None):
        raise ValueError("SOURCE_DATA_PATH is not defined in config.")
    if not getattr(config.DATA, "TARGET_DATA_PATH", None):
        raise ValueError("TARGET_DATA_PATH is not defined in config.")

    source_path = Path(config.DATA.SOURCE_DATA_PATH)
    target_path = Path(config.DATA.TARGET_DATA_PATH)
    if not (source_path / "val").exists() or not (target_path / "val").exists():
...
if __name__ == "__main__":
    ...
    args = parser.parse_args([
        ...
        '--continuous',
        '--max_batches', '200',
        '--out_dir', '/data/scavone/graph_stats_test',
        '--split', 'train',
        '--config', '3d/configs/synth_3D.yaml'
    ])
